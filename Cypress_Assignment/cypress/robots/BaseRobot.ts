...
export abstract class BaseEyes {
  ...
  seesTextWithxpath(xpath: string, text: string) {
...
export class BaseHands {
  getLastDisplayedItemDetails(
    ...
    return cy
      ...
      .last()
      .then(($lastItem) => {
        const itemName = $lastItem.find(itemSelector).text();
        const itemPrice = $lastItem.find(priceSelector).text();
        return cy.wrap({ itemName, itemPrice }); // Return Cypress chainable wrapped object
      });
  }

  logItemDetails(itemDetails: { itemName: string; itemPrice: string }) {
    cy.log(
      `Last Displayed Item: ${itemDetails.itemName} - Price: ${itemDetails.itemPrice}`
    );
  }
  clickDomElementwithXpath(xpath: string) {
